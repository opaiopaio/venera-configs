class Ehentai extends ComicSource {
  ...
  // update url
  url = "https://cdn.jsdelivr.net/gh/opaiopaio/venera-configs@main/ehentai.js";
  ...
  account = {
    ...
    loginWithCookies: {
      ...
      validate: async (values) => {
        ...
        let res = await Network.get("https://forums.e-hentai.org/", {
  ...
  async getGalleries(url, isLeaderBoard) {
    ...
    for (let item of document.querySelectorAll("table.itg.gltc > tbody > tr")) {
      try {
        ...
        let stars = this.getStarsFromPosition(
        ...
        try {
          if (url.includes("/favorites.php")) {
            pages = Number(
              item.children[
                1 + t
              ].children[1].children[1].children[1].children[1].text.match(
                /\d+/,
              )[0],
            );
          } else {
            pages = Number(
              item.children[3 + t].children[1].text.match(/\d+/)[0],
            );
        ...
        let tags = [];
        let language = null;
        for (let node of item.children[2 + t].children[0].children[1]
          .children) {
    ...
    for (let item of document.querySelectorAll("table.itg.glte > tbody > tr")) {
      try {
        ...
        let coverPath =
          ...
          "";
        let stars = this.getStarsFromPosition(
          item.querySelector("td.gl2e > div > div.gl3e > div.ir")?.attributes[
            "style"
          ] ?? "",
        );
        let link =
    ...
    for (let item of document.querySelectorAll("table.itg.gltm > tbody > tr")) {
      try {
        ...
        let coverPath = item.querySelector("td.gl2m > div > div > img")
          ?.attributes["src"];
        if (coverPath[0] === "d") {
          coverPath = item.querySelector("td.gl2m > div > div > img")
            ?.attributes["data-src"];
        }
        let stars = this.getStarsFromPosition(
    ...
    ranking: {
      ...
      load: async (option, page) => {
        let res = await this.getGalleries(
          `https://e-hentai.org/toplist.php?tl=${option}&p=${page - 1}`,
          true,
        );
  ...
  comic = {
      ...
      for (let tr of document.querySelectorAll(
        ...
        tags.set(
          ...
          tr.children[1].children.map(
            (e) =>
              e.children[0].attributes["onclick"].split(":")[1].split("'")[0],
          ),
        );
      }
    ...
    getKey: async (url) => {
      ...
      let script = document
        .querySelectorAll("script")
        .find((e) => e.text.includes("showkey"));
      if (script) {
      ...
      script = document
        .querySelectorAll("script")
        .find((e) => e.text.includes("mpvkey"))?.text;
      document.dispose();
      if (script) {
        let mpvkey = script
          .split(";")
      ...
      let getImageFromApi = async (nl) => {
        if (key.mpvkey) {
          ...
          if (page < first.thumbnails.length) {
            ...
            let thumbnails = await this.comic.loadThumbnails(
              comicId,
    ...
    parseComments: (document) => {
      ...
      for (let c of document.querySelectorAll("div.c1")) {
        ...
        let isUp =
          c.querySelector(`a#comment_vote_up_${id}`)?.attributes["style"]
            ?.length > 0;
        let isDown =
    ...
    archive: {
      getArchives: async (cid) => {
        ...
        let res = await Network.get(
          `${this.baseUrl}/archiver.php?gid=${gid}&token=${token}`,
          {},
        );
        if (res.status !== 200) {
          throw `Invalid status code: ${res.status}`;
          ...
          for (let cell of hathCells) {
            ...
            if (link) {
              ...
              if (resolutionMatch) {
                ...
                let size =
                  paragraphs.length > 1 ? paragraphs[1].text : "Unknown";
                let cost =
                  paragraphs.length > 2 ? paragraphs[2].text : "Unknown";
        ...
        if (origin) {
          let originCost =
            origin.querySelector("div > strong")?.text || "Unknown";
          let originSize =
            origin.querySelector("p > strong")?.text || "Unknown";
          archives.push({
            id: "0",
            title: "Original",
        ...
        if (resample) {
          let resampleCost =
            resample.querySelector("div > strong")?.text || "Unknown";
          let resampleSize =
            resample.querySelector("p > strong")?.text || "Unknown";
          archives.push({
            id: "1",
            title: "Resample",
      ...
      getDownloadUrl: async (cid, aid) => {
        ...
        if (aid.startsWith("h@h_")) {
          ...
          let hathRes = await Network.post(
            `${this.baseUrl}/archiver.php?gid=${gid}&token=${token}`,
            {
              "Content-Type": "application/x-www-form-urlencoded",
            },
        ...
        let test = await Network.sendRequest(
          "HEAD",
          resultLink,
          {
            http_client: "dart:io",
