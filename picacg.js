class Picacg extends ComicSource {
  ...
  url = "https://cdn.jsdelivr.net/gh/opaiopaio/venera-configs@main/picacg.js";
  ...
  categoryComics = {
    load: async (category, param, options, page) => {
      ...
      let res = await Network.get(
        ...
        this.buildHeaders(
          "GET",
          `comics?page=${page}&${type}=${encodeURIComponent(category)}&s=${options[0]}`,
          this.loadData("token"),
        ),
      );
      if (res.status === 401) {
    ...
    ranking: {
      ...
      load: async (option, page) => {
        let res = await Network.get(
          ...
          this.buildHeaders(
            "GET",
            `comics/leaderboard?tt=${option}&ct=VC`,
            this.loadData("token"),
          ),
        );
        if (res.status === 401) {
  ...
  favorites = {
    ...
    loadComics: async (page, folder) => {
      ...
      let res = await Network.get(
        ...
        this.buildHeaders(
          "GET",
          `users/favourite?page=${page}&s=${sort}`,
          this.loadData("token"),
        ),
      );
      if (res.status === 401) {
  ...
  comic = {
    ...
    loadInfo: async (id) => {
      ...
      try {
        [info, eps, related] = await Promise.all([
          infoLoader(),
          epsLoader(),
          relatedLoader(),
        ]);
    ...
    loadEp: async (comicId, epId) => {
      ...
      while (true) {
        let res = await Network.get(
          ...
          this.buildHeaders(
            "GET",
            `comics/${comicId}/order/${epId}/pages?page=${i}`,
            this.loadData("token"),
          ),
        );
        if (res.status !== 200) {
    ...
    loadComments: async (comicId, subId, page, replyTo) => {
      function parseComment(c) {
        return new Comment({
          ...
          avatar: c._user.avatar
            ? c._user.avatar.fileServer + "/static/" + c._user.avatar.path
            : undefined,
          id: c._id,
          content: c.content,
          isLiked: c.isLiked,
