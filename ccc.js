...
class CCC extends ComicSource {
  ...
  // update url
  url = "https://cdn.jsdelivr.net/gh/opaiopaio/venera-configs@main/ccc.js";
  ...
  account = {
    ...
    login: async (account, pwd) => {
      let res = await Network.get(
        `${this.apiUrl}/recaptcha#${randomInt(0, 999)}`,
        await this.getApiHeaders(true),
      ); //使用隨機fragment來強制url重新加載
      const captcha = JSON.parse(res.body);
      if (captcha.message != "ok") {
        throw "登錄失敗";
      }
      const captcha_code = await UI.showInputDialog(
        "驗證碼",
        null,
        this.base64ToArrayBuffer(captcha.result.img),
      );
  ...
  favorites = {
    ...
    addOrDelFavorite: async (comicId, folderId, isAdding, favoriteId) => {
      ...
      const res = await Network.put(
        `${this.apiUrl}/book/${comicId}/collect`,
        await this.getApiHeaders(),
        { is_collected: isAdding },
      );
    ...
    loadComics: async (page, folder) => {
      return this.parseComics(
        `${this.apiUrl}/bookcase/collections?page=${page}&rows_per_page=20&sort_by=updated_at&class=2`,
      );
    },
  ...
  comic = {
    freeRead: (data) => {
      ...
      if (!data["is_free"]) {
        if (data["sales_plan"] != 0) {
          if (
            (data["is_coin_buy"] || data["is_point_buy"]) &&
            !data["is_buy"]
          ) {
            if (
              (data["is_coin_rent"] || data["is_point_rent"]) &&
              !data["is_rent"]
            ) {
              free_read = false;
            }
    ...
    loadInfo: async (id) => {
      ...
      const chapter_res = await Network.get(
        `${this.apiUrl}/book/${id}/chapter`,
        await this.getApiHeaders(),
      );
      const chapterData = JSON.parse(chapter_res.body)["data"];
      const chapters = {};
      for (let c of chapterData["chapters"]) {
        chapters[c["id"].toString()] =
          `${!this.comic.freeRead(c) ? "[付費]" : ""}${c["vol_name"]}-${c["name"]}`;
      }
      const recommend_res = await Network.get(
        `${this.apiUrl}/book/${id}/recommend`,
        await this.getApiHeaders(),
      );
      ...
      if (res.status == 403) {
        UI.showDialog("提示", "該章節需付費后閲讀", [
          ...
          },
          {
            ...
            callback: () => {
              UI.launchUrl(
                `https://www.creative-comic.tw/zh/book/${comicId}/content`,
              );
            },
          },
        ]);
    ...
    onImageLoad: async (url, comicId, epId) => {
      const res = await Network.get(
        `${this.apiUrl}/book/chapter/image/${url}`,
        await this.getApiHeaders(),
      );
      const encryptedKey = Convert.decodeBase64(
        JSON.parse(res.body)["data"]["key"],
      );
      let token = this.loadData("token");
      if (token == null) {
      ...
      const decryptedKey = new Uint8Array(
        Convert.decryptAesCbc(encryptedKey, pageKey, pageIv),
      );
      const padLen = decryptedKey[decryptedKey.length - 1];
      const [key, iv] = Convert.decodeUtf8(
        decryptedKey.slice(0, decryptedKey.length - padLen).buffer,
      ).split(":");
      return {
        ...
        onResponse: function (buffer) {
          ...
          const decrypted = new Uint8Array(
            Convert.decryptAesCbc(buffer, hexToBytes(key), hexToBytes(iv)),
          );
          const padLen_ = decrypted[decrypted.length - 1];
          const base64 = Convert.decodeUtf8(
            decrypted.slice(0, decrypted.length - padLen_).buffer,
          );
          const base64Data = base64.split(",")[1] || base64;
          return Convert.decodeBase64(base64Data);
        },
    ...
    loadComments: async (comicId, subId, page, replyTo) => {
      const res = await Network.get(
        `${this.apiUrl}/book/${comicId}/reply?page=${page}&rows_per_page=20&sort_by=created_at&descending=true#${randomInt(0, 999)}`,
    ...
    sendComment: async (comicId, subId, content, replyTo) => {
      ...
      const boundary =
        "----geckoformboundary" +
        Math.random().toString(16).replace(".", "a") +
        Math.random().toString(16).replace(".", "a");
      const body =
        `--${boundary}\r\n` +
    ...
    likeComment: async (comicId, subId, commentId, isLike) => {
      ...
      const res = await Network.put(
        `${this.apiUrl}/book/reply/${commentId.split("@")[0]}/like`,
        await this.getApiHeaders(),
        { is_like: isLike ? 1 : 0 },
      );
      if (JSON.parse(res.body)["message"] != "ok") {
