class JM extends ComicSource {
  ...
  // update url
  url = "https://cdn.jsdelivr.net/gh/opaiopaio/venera-configs@main/jm.js";
  ...
  convertData(input, secret) {
    let key = Convert.encodeUtf8(
      Convert.hexEncode(Convert.md5(Convert.encodeUtf8(secret))),
    );
  ...
  async dailyCheckIn(isTask = false) {
    ...
    try {
      ...
      const checkRecordRes = await this.get(
        `${this.baseUrl}/daily?user_id=${uid}`,
      );
      ...
      const checkResultRes = await this.post(
        `${this.baseUrl}/daily_chk`,
        `user_id=${uid}&daily_id=${daily_id}`,
      );
  ...
  categoryComics = {
    ...
    load: async (category, param, options, page) => {
      if (category !== "每週必看") {
        ...
        let res = await this.get(
          `${this.baseUrl}/categories/filter?o=${options[0]}&c=${param}&page=${page}`,
        );
        ...
        let res = await this.get(
          `${this.baseUrl}/week/filter?id=${options[0]}&type=${options[1]}&page=0`,
        );
        let data = JSON.parse(res);
  ...
  favorites = {
    ...
    addOrDelFavorite: async (comicId, folderId, isAdding, favoriteId) => {
      if (isAdding) {
        ...
        await this.post(
          `${this.baseUrl}/favorite_folder`,
          `type=move&folder_id=${folderId}&aid=${comicId}`,
        );
      } else {
        await this.post(`${this.baseUrl}/favorite`, `aid=${comicId}`);
      }
    ...
    loadComics: async (page, folder) => {
      ...
      let res = await this.get(
        `${this.baseUrl}/favorite?folder_id=${folder}&page=${page}&o=${order}`,
      );
      let json = JSON.parse(res);
  ...
  comic = {
    ...
    loadInfo: async (id) => {
      ...
      let tags = data.tags ?? [];
      let related = data["related_list"].map(
        (e) =>
          new Comic({
            id: e.id.toString(),
            title: e.name,
    ...
    loadComments: async (comicId, subId, page, replyTo) => {
      let res = await this.get(
        `${this.baseUrl}/forum?mode=manhua&aid=${comicId}&page=${page}`,
      );
      let json = JSON.parse(res);
      const pageSize = 6;
      return {
        comments: json.list.map(
          (e) =>
            new Comment({
    ...
    sendComment: async (comicId, subId, content, replyTo) => {
      let res = await this.post(
        ...
        `aid=${comicId}&comment=${encodeURIComponent(content)}&status=undefined`,
      );
      let json = JSON.parse(res);
      if (json.status === "fail") {
