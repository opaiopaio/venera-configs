class MXS extends ComicSource {
  ...
  version = "1.0.0";
  minAppVersion = "1.5.0";
  url = "https://cdn.jsdelivr.net/gh/opaiopaio/venera-configs@main/mxs.js";
    ...
    for (let item of items) {
      ...
      const description =
        item
          ...
          ?.text?.replace(/^更新/, "")
          ?.replace(/\s+/g, " ")
          ?.trim() || item.querySelector(".zl")?.text?.trim();
  ...
  parseHotComicList(items) {
    ...
    for (let item of items) {
      ...
      const clickCount = item
        .querySelector(".info .subtitle span a")
        ?.text?.trim();
  ...
  explore = [
    {
      ...
      load: async (page) => {
        ...
        const updateSection = {
          title: "最近更新",
          comics: this.parseComicList(
            doc.querySelectorAll(".index-manga .mh-item"),
          ),
          viewMore: {
            page: "category",
        ...
        const hotSection = {
          title: "热门漫画",
          comics: this.parseHotComicList(
            doc.querySelectorAll(".index-original .index-original-list li"),
          ),
          viewMore: {
            page: "category",
        ...
        const endSection = {
          title: "完结优选",
          comics: this.parseComicList(
            doc.querySelectorAll(".box-body .mh-item"),
          ),
          viewMore: {
            page: "category",
  ...
  categoryComics = {
    ...
    load: async (category, param, options, page) => {
      ...
      if (category === "排行榜") {
        ...
        comics = this.parseComicList(
          targetList.querySelectorAll(".mh-item.horizontal, .mh-itme-top"),
        );
      } else {
        // 普通分类处理
        comics = this.parseComicList(
          doc.querySelectorAll(".mh-list.col7 .mh-item"),
        );
      }
  ...
  comic = {
    ...
    loadInfo: async (id) => {
      ...
      for (let elem of tipElems) {
        const text = elem.text;
        if (text.includes("状态："))
          status = elem.querySelector("span")?.text?.trim();
        if (text.includes("地区："))
          area = elem.querySelector("a")?.text?.trim();
        if (text.includes("更新时间："))
          updateTime = elem.text.replace("更新时间：", "").trim();
        if (text.includes("点击："))
          clickCount = elem.text.replace("点击：", "").trim();
      }
      ...
      // 提取评论和推荐漫画
      const comments = this.parseCommentList(
        doc.querySelectorAll(".view-comment-main .postlist li.dashed"),
      );
      const recommend = this.parseComicList(
        doc.querySelectorAll(".index-manga .mh-item"),
      );
