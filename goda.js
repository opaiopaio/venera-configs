...
class Goda extends ComicSource {
  ...
  // update url
  url = "https://cdn.jsdelivr.net/gh/opaiopaio/venera-configs@main/goda.js";
  ...
  comic = {
    ...
    loadInfo: async (id) => {
      ...
      const jsonRes = await Network.get(
        `${this.apiUrl}/manga/get?mid=${mangaId}&mode=all&t=${Date.now()}`,
        this.headers,
      );
    ...
    loadEp: async (comicId, epId) => {
      ...
      const res = await Network.get(
        `${this.apiUrl}/chapter/getinfo?m=${ids[0]}&c=${ids[1]}`,
        this.headers,
      );
      if (res.status !== 200) {
        throw `Invalid status code: ${res.status}`;
