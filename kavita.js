...
class Kavita extends ComicSource {
  ...
  url = "https://cdn.jsdelivr.net/gh/opaiopaio/venera-configs@main/kavita.js";
  ...
  comic = {
    ...
    loadInfo: async (id) => {
      ...
      const metadata = await this.getJson(`/api/Series/metadata`, {
        seriesId: id,
      });
      const volume = await this.getJson(`/api/Series/volumes`, {
        seriesId: id,
      });
      ...
      const info = new ComicDetails({
        ...
        cover: this.buildUrl(`/api/Image/series-cover`, {
          seriesId: id,
          apiKey: apiKey,
        }),
    ...
    loadEp: async (comicId, epId) => {
      const data = await this.getJson(`/api/Series/chapter`, {
        chapterId: epId,
      });
      ...
      return {
        images: Array.from({ length: page }, (_, i) =>
          this.buildUrl(`/api/Reader/image`, {
            chapterId: epId,
            page: i,
            apiKey: apiKey,
            extractPdf,
