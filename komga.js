...
class Komga extends ComicSource {
  ...
  url = "https://cdn.jsdelivr.net/gh/opaiopaio/venera-configs@main/komga.js";
  ...
  get authToken() {
    ...
    const encoded = Convert.encodeBase64(
      Convert.encodeUtf8(`${username}:${password}`),
    );
  ...
  account = {
    login: async (account, pwd) => {
      ...
      const basic = Convert.encodeBase64(
        Convert.encodeUtf8(`${account}:${pwd}`),
      );
      const token =
        typeof basic === "string" ? basic : Convert.decodeUtf8(basic);
      const res = await Network.get(this.buildUrl("/api/v2/users/me"), {
  ...
  explore = [
    {
      ...
      load: async () => {
        await this.refreshReferenceData(false);
        const feeds = {};
        const latest = await this.fetchSeriesList("/api/v1/series/latest", {
          size: 12,
          page: 0,
        });
        if (latest.comics.length) feeds["最新上架"] = latest.comics;
        const updated = await this.fetchSeriesList("/api/v1/series/updated", {
          size: 12,
          page: 0,
        });
  ...
  categoryComics = {
    load: async (category, param, options, page) => {
      ...
      const defaultSort =
        category === "all" ? "created,desc" : "metadata.lastModified,desc";
      ...
      if (category === "collection" && param) {
        const list = await this.fetchSeriesList(
          `/api/v1/collections/${param}/series`,
          query,
        );
        return {
          comics: list.comics,
  ...
  search = {
    load: async (keyword, options, page) => {
      const pageIndex = Math.max(0, (page || 1) - 1);
      const sortValue = this.extractOption(
        options,
        0,
        "metadata.lastModified,desc",
      );
      const query = {
        page: pageIndex,
        size: 30,
  ...
  comic = {
    loadInfo: async (id) => {
      ...
      const tags = Array.isArray(series?.booksMetadata?.tags)
        ? series.booksMetadata.tags
        : [];
      ...
      const images = list
        ...
        .map((page) => {
          ...
          return this.buildUrl(
            `/api/v1/books/${bookId}/pages/${number}`,
            zeroBased ? { zero_based: true } : null,
          );
        });
      return { images };
    },
  ...
  async refreshReferenceData(force) {
    ...
    try {
      const [libraries, tags, languages, collections, genres] =
        await Promise.all([
      ...
      const collectionPage =
        collections && typeof collections === "object" ? collections : null;
      const collectionList = Array.isArray(collectionPage?.content)
  ...
  async fetchSeriesList(path, query) {
    ...
    const comics = content
      .map((item) => this.parseSeries(item))
      .filter(Boolean);
    return {
      comics,
  ...
  parseSeries(series) {
    ...
    if (Array.isArray(series?.booksMetadata?.tags))
      tags.push(...series.booksMetadata.tags);
    const description =
      series?.booksMetadata?.summary || metadata.summary || "";
    return new Comic({
      id: series.id,
      title,
  ...
  collectAuthors(authors) {
    ...
    return this.uniqueArray(
      authors.map((author) => author?.name).filter(Boolean),
    );
  }
  ...
  compareBooks(a, b) {
    const aSort =
      typeof a?.metadata?.numberSort === "number" ? a.metadata.numberSort : NaN;
    const bSort =
      typeof b?.metadata?.numberSort === "number" ? b.metadata.numberSort : NaN;
