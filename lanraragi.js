...
class Lanraragi extends ComicSource {
  ...
  version = "1.2.0";
  minAppVersion = "1.4.0";
  url = "https://cdn.jsdelivr.net/gh/opaiopaio/venera-configs@main/lanraragi.js";
  ...
  get headers() {
    ...
    if (raw) {
      headers.Authorization =
        "Bearer " + Convert.encodeBase64(Convert.encodeUtf8(raw));
    }
    return headers;
  }
  ...
  search = {
    load: async (keyword, options, page = 1) => {
      ...
      const searchKey =
        "search_start_" + encodeURIComponent(String(keyword || ""));
      let start = 0;
      if (page === 1) {
  ...
  comic = {
    loadInfo: async (id) => {
      ...
      for (const key of Object.keys(tagsObj)) {
        ...
        for (const val of arr) {
          ...
          if (String(key).toLowerCase() === "source") {
            ...
            else if (!/^https?:\/\//i.test(corrected))
      ...
      let stars = this._toStarsFromValue(
    ...
    loadThumbnails: async (id, next) => {
      ...
      for (let i = 1; i <= pagecount; i++)
        thumbnails.push(
          `${this.baseUrl}/api/archives/${id}/thumbnail?page=${i}`,
        );
    ...
    starRating: async (id, rating) => {
      ...
      tagsArr = tagsArr.filter(
        (t) =>
          !(typeof t === "string" && t.toLowerCase().startsWith("rating:")),
      );
