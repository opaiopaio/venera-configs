class Comick extends ComicSource {
  ...
  minAppVersion = "1.4.0";
  // update url
  url = "https://cdn.jsdelivr.net/gh/opaiopaio/venera-configs@main/comick.js";

  settings = {
    domains: {
      title: "主页源",
      type: "select",
      options: [{ value: "comick.art" }],
      default: "comick.art",
    },
  };
  ...
  transformBookList(bookList, descriptionPrefix = "更新至：") {
    return bookList.map((book) => ({
      ...
      cover: book.default_thumbnail
        ? book.default_thumbnail
        : book.full_image_path
          ? book.full_image_path
  ...
  getFormattedManga(manga) {
    return {
      ...
      cover: manga.default_thumbnail
        ? manga.default_thumbnail
        : manga.full_image_path
          ? manga.full_image_path
          : "https://comick.art/images/default-thumbnail.webp",
      tags: [
  ...
  categoryComics = {
    load: async (category, param, options, page) => {
      ...
      if (options[1] && options[1] !== "-全部") {
        params.push(
          `country[]=${encodeURIComponent(options[1].split("-")[0])}`,
        );
      }
  ...
  comic = {
    ...
    loadInfo: async (id) => {
      ...
      let res = await Network.get(`https://comick.art/comic/${cId}`, headers);
      if (res.status !== 200) {
        throw "Invalid status code: " + res.status;
      }
      ...
      let load_chapter = async (slug, comicData) => {
        ...
        while (page <= lastPage) {
          ...
          let resCh = await Network.get(
            (url = url),
            (headers = Comick.getRandomHeaders()),
          );
        ...
        langBuckets.forEach((items, langCode) => {
          ...
          let displayLang =
            Comick.language_dict[langCode] || langCode || "未知语言";
      ...
      let cover = comicData.default_thumbnail
        ? comicData.default_thumbnail
        : comicData.full_image_path
          ? comicData.full_image_path
          : "https://comick.art/images/default-thumbnail.webp";
      ...
      let extractSlugs = (comicData) => {
        try {
          ...
          let slugs = genres
            .map((genre) => genre?.md_genres?.slug)
      ...
      let fallbackUpdate = comicData?.last_chapter
      ...
      try {
        ...
        if (Array.isArray(temp)) {
          ...
          updateTime =
            typeof temp[1] === "string" && temp[1].length > 0
              ? temp[1]
              : updateTime;
        }
      } catch (error) {
