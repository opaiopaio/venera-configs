class Nhentai extends ComicSource {
  ...
  _headers(extra = {}) {
    ...
    let apiKey = this.loadData("apiKey") || this.loadSetting("apiKey");
    if (apiKey) {
  ...
  parseComicFromApi(item) {
    ...
    if (tagIds.indexOf(12227) !== -1) {
      lang = "English";
    } else if (tagIds.indexOf(6346) !== -1) {
      lang = "日本語";
    } else if (tagIds.indexOf(29963) !== -1) {
      lang = "中文";
    }
    let tagsRes = [];
  ...
  comic = {
    ...
    loadInfo: async (id) => {
      ...
      if (apiRes.status === 200) {
        ...
        let related = (data.related || []).map((e) =>
          this.parseComicFromApi(e),
        );
