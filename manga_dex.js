...
class MangaDex extends ComicSource {
  ...
  api = {
    parseComic: (data) => {
      ...
      let cover = data["relationships"].find(
        (e) => e["type"] === "cover_art",
      )?.["attributes"]["fileName"];
      if (cover) {
  ...
  categoryComics = {
    load: async (category, param, options = [], page = 1) => {
      ...
      let maxPage = total
        ? Math.ceil(total / this.comicsPerPage)
        : comics.length < this.comicsPerPage
          ? page
          : page + 1;
      return {
        comics: comics,
  ...
  comic = {
    ...
    getChapters: async (id) => {
      let res = await fetch(
        `https://api.mangadex.org/manga/${id}/feed?limit=500&translatedLanguage[]=en&order[chapter]=asc`,
      );
      if (!res.ok) {
        throw new Error("Network response was not ok");
      }
    ...
    loadEp: async (comicId, epId) => {
      ...
      if (image_quality == "Original") {
        for (let image of data["chapter"]["data"]) {
          images.push(
            `https://uploads.mangadex.org/data/${data["chapter"]["hash"]}/${image}`,
          );
        }
      } else {
        for (let image of data["chapter"]["dataSaver"]) {
          images.push(
            `https://uploads.mangadex.org/data-saver/${data["chapter"]["hash"]}/${image}`,
          );
        }
      }
      return {
      ...
      for (let msg of messageElements) {
        ...
        if (contentElement) {
          ...
          content = content.replace(
            /<div class="bbCodeBlock-expandLink[^>]*>.*?<\/div>/gi,
            "",
          );
          ...
          // Spoiler Style
          content = content.replace(
            /<button[^>]*class="[^"]*bbCodeSpoiler-button[^"]*"[^>]*>([\s\S]*?)<\/button>/gi,
          ...
          content = content.replace(/<\/?([a-z0-9]+)[^>]*>/gi, (match, tag) => {
            ...
            if (
              ["div", "p"].includes(tag.toLowerCase()) &&
              match.startsWith("</")
            ) {
              return "<br>";
            }
