...
class HotManga extends ComicSource {
  ...
  get headers() {
    return {
      Authorization: this.loadData("token")
  ...
  explore = [
    {
      ...
      load: async () => {
        ...
        res["每周免费漫画排行"] =
          data["results"]["rankWeeklyFreeComics"]["list"].map(parseComic);
        res["每周付费漫画排行"] =
          data["results"]["rankWeeklyChargeComics"]["list"].map(parseComic);
        res["付费漫画更新"] =
          data["results"]["updateWeeklyChargeComics"]["list"].map(parseComic);
        res["免费漫画更新"] =
          data["results"]["updateWeeklyFreeComics"]["list"].map(parseComic);
        return res;
      },
    },
  ];
  ...
  comic = {
    loadInfo: async (id) => {
      ...
      let getFavoriteStatus = async (id) => {
        let res = await Network.get(
          `${this.apiUrl}/api/v3/comic2/${id}/query`,
          this.headers,
        );
        if (res.status !== 200) {
          throw `Invalid status code: ${res.status}`;
      ...
      let tags = comicData.theme
        .map((e) => e?.name)
        .filter((name) => name !== undefined && name !== null);
      let updateTime = comicData.datetime_updated
        ? comicData.datetime_updated
        : "";
