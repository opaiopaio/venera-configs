class Happy extends ComicSource {
  ...
  version = "1.0.0";
  minAppVersion = "1.6.0";
  url = "https://cdn.jsdelivr.net/gh/opaiopaio/venera-configs@main/happy.js";
  ...
  comic = {
    ...
    loadInfo: async (id) => {
      ...
      // 从HTML提取JSON数据
      const jsonInHtml = res.body.match(
        /<mip-data>\s*<script type="application\/json">\s*([\s\S]*?)<\/script>\s*<\/mip-data>/i,
      )?.[1];
      ...
      const subTitle = doc
        .querySelector(".mg-sub-title")
        ?.text.replace(/,/g, "／")
        .trim();
      const cover = doc.querySelector("mip-img").attributes.src;
      const authorRaw = doc
        .querySelectorAll(".mg-sub-title a")
        .map((a) => a.text.trim())
        .join(",");
      const authors = this.formatAuthor(authorRaw);
      const author = authors.join(" | ");
      const genres = doc
        .querySelectorAll(".mg-cate a")
        .map((a) => a.text.trim())
        .filter((a) => a);
      const descRaw = doc.querySelector("mip-showmore")?.text.trim();
      const description = subTitle
        ? [descRaw, `别名：${subTitle}`].filter((a) => a).join("\n\n")
        : descRaw;
      const updateTimeRaw = doc
        .querySelector(".update-time .time")
        ?.text.trim();
    ...
    loadComments: async (comicId, subId, page, replyTo) => {
      return await this.loadCommentsCommon(
        comicId,
        null,
        page,
        replyTo,
