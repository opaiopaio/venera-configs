...
class ManHuaRen extends ComicSource {
      ...
      load: async (page) => {
        ...
        for (let i = 0; i < lists.length; i++) {
          ...
          for (let j = 0; j < items.length; j++) {
            ...
            let tip =
              item.querySelector(".manga-list-1-tip") ||
              item.querySelector(".manga-list-2-tip");
  ...
  search = {
    ...
    load: async (keyword, options, page) => {
      ...
      for (let item of list) {
        ...
        let status = item
          .querySelector(".book-list-info-bottom-right-font")
          ?.text?.trim();
  ...
  comic = {
    ...
    loadInfo: async (id) => {
      ...
      let title =
        doc.querySelector("p.detail-main-info-title")?.text?.trim() ||
      ...
      let coverEl =
        doc.querySelector(".detail-main-cover img") ||
      ...
      if (authorContainer) {
        ...
        let metaAuthor = doc.querySelector('meta[name="Author"]')?.attributes
          ?.content;
        if (metaAuthor) {
          author = metaAuthor.includes(":")
            ? metaAuthor.split(":").pop().trim()
            : metaAuthor.trim();
        }
      }

      let status =
        doc.querySelector(".detail-list-title-1")?.text?.trim() || "未知状态";
      ...
      let updateTime =
        doc.querySelector(".detail-list-title-3")?.text?.trim() || "";

      let starValue = null;
      let starElement = doc.querySelector(".detail-main-info-star");
      if (
        starElement &&
        starElement.attributes &&
        starElement.attributes["class"]
      ) {
      ...
      let selectorItems = doc.querySelectorAll(
        ".detail-selector .detail-selector-item",
      );
          ...
          if (listId) {
            ...
            if (listEl) {
              ...
              for (let link of links) {
                let href = link.attributes["href"];
                let title =
                  link.text?.trim() || link.attributes["title"]?.trim();
                if (href && title) {
      ...
      let parseRecommends = (htmlContent) => {
        let recs = [];
        let recPattern =
          /<li[^>]*class=["'][^"']*(?:list-comic|rec|recommend)[^"']*["'][^>]*>[\s\S]*?<a[^>]*href=["']([^"']+)["'][^>]*>[\s\S]*?<img[^>]*src=["']([^"']+)["'][^>]*>[^<]*<\/a>[\s\S]*?<a[^>]*>\s*([^<]+)\s*<\/a>/gi;
        let m;
        let count = 0;
    ...
    loadComments: async (comicId, subId, page, replyTo) => {
      ...
      if (replyTo) {
        let target = data.find(
          (item) => item.Id.toString() === targetCommentId,
        );
        if (target && target.ToPostShowDataItems) {
          comments = target.ToPostShowDataItems.map(
            (item) =>
              new Comment({
                id: item.Id.toString(),
        ...
        comments = data.map(
          (item) =>
            new Comment({
              ...
              likeCount: item.PraiseCount,
              isLiked: item.IsPraise,
              replyCount: item.ToPostShowDataItems
                ? item.ToPostShowDataItems.length
                : 0,
            }),
        );
        if (comments == []) {
