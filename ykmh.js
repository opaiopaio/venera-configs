...
class YKMHSource extends ComicSource {
  ...
  version = "1.0.0";
  minAppVersion = "1.4.0";
  url = "https://cdn.jsdelivr.net/gh/opaiopaio/venera-configs@main/ykmh.js";
  ...
  explore = [
    {
      ...
      load: async (page) => {
        ...
        function parseHotCarousel(html) {
          let hotComics = [];
          let carouselPattern =
            /<div class="sub-item">\s*<a href="([^"]+)" target="_blank"><img src="([^"]+)" alt="[^"]*"><\/a>\s*<div class="carousel-caption">\s*([^<]+)\s*<\/div>/g;
          let match;
          ...
          if (hotComics.length === 0) {
            let keywordPattern =
              /<li data-key="(\d+)"><a href="(https:\/\/www\.ykmh\.net\/manhua\/[^"]+)"[^>]*>([^<]+)<\/a><\/li>/g;
        ...
        function parseLatestComics(html) {
          let latestComics = [];
          let comicPattern =
            /<li data-key="(\d+)"><a class="image-link" href="([^"]+)" title="([^"]+)"><img src="([^"]+)"[^>]*><span class="tip"><p>([^<]*)<\/p><\/span><\/a><p><a href="[^"]*" title="[^"]*">([^<]+)<\/a><\/p>/g;
          let match;
  ...
  categoryComics = {
    load: async (category, param, options, page) => {
      ...
      function parseComicsList(html) {
        let comics = [];
        let comicPattern =
          /<li class="list-comic" data-key="(\d+)">\s*<a class="comic_img"\s+href="([^"]+)"><img src="([^"]+)" alt="([^"]*)"[^>]*><\/a>\s*<span class="comic_list_det"[^>]*>\s*<h3><a href="[^"]*">([^<]+)<\/a><\/h3>/g;
      ...
      let maxPage = 1;
      let pagePattern =
        /<li class="last"><a href="[^"]*\/(\d+)\/" data-page="\d+">尾页<\/a><\/li>/;
      let pageMatch = res.body.match(pagePattern);
      if (pageMatch) {
        maxPage = parseInt(pageMatch[1]);
      } else {
        let altPagePattern =
          /<li class="last"><a href="[^"]*" data-page="(\d+)">尾页<\/a><\/li>/;
  ...
  search = {
    load: async (keyword, options, page) => {
      ...
      function parseSearchResults(html) {
        let comics = [];
        let comicPattern =
          /<li class="list-comic" data-key="(\d+)"><a class="image-link"\s+href="([^"]+)"\s+title="([^"]+)"><img src="([^"]+)"[^>]*><\/a>\s*<p><a href="[^"]*"[^>]*>([^<]+)<\/a><\/p>\s*<p class="auth"><a href="[^"]*">([^<]*)<\/a><\/p>\s*<p class="newPage">([^<]*)<\/p>/g;
      ...
      let maxPage = 1;
      let pagePattern =
        /<li class="last"><a href="[^"]*page=(\d+)"[^>]*>尾页<\/a><\/li>/;
      let pageMatch = res.body.match(pagePattern);
      if (pageMatch) {
        maxPage = parseInt(pageMatch[1]);
      } else {
  ...
  comic = {
    ...
    loadInfo: async (id) => {
        ...
        try {
          ...
          try {
            let titleMatch = html.match(
              /<div class="BarTit" id="comicName">([^<]+)<\/div>/,
            );
          ...
          try {
            let coverMatch = html.match(
              /<div class="pic" id="Cover">\s*<mip-img src="([^"]+)"/,
            );
          ...
          try {
            let authorMatch = html.match(
              /<p class="txtItme">\s*<span class="icon icon01"><\/span>\s*<a href="[^"]*">([^<]+)<\/a>\s*<\/p>/,
            );
            if (authorMatch && authorMatch[1]) {
              author = authorMatch[1].trim();
            } else {
              let authorMatch2 = html.match(
                /<span class="icon icon01"><\/span>\s*<a href="[^"]*">([^<]+)<\/a>/,
              );
          ...
          try {
            let descMatch = html.match(
              /<mip-showmore[^>]*id="showmore-des">\s*([^<]+(?:<[^>]+>[^<]*<\/[^>]+>[^<]*)*?)\s*<\/mip-showmore>/,
            );
      ...
      function parseChapters(html) {
        ...
        try {
          ...
          let chapterGroupsPattern =
            /<div class="comic-chapters">[\s\S]*?<span class="Title">([^<]+)<\/span>[\s\S]*?<ul id="chapter-list-(\d+)"[^>]*>([\s\S]*?)<\/ul>/g;
          ...
          while ((groupMatch = chapterGroupsPattern.exec(html)) !== null) {
            try {
              if (groupMatch && groupMatch[1] && groupMatch[3]) {
                ...
                let chapterPattern =
                  /<li>\s*<a href="([^"]+)"[^>]*>\s*<span>([^<]+)<\/span>\s*<\/a>\s*<\/li>/g;

                let chapterMatch;
                while (
                  (chapterMatch = chapterPattern.exec(groupContent)) !== null
                ) {
                  try {
      ...
      function parseRecommends(html) {
        ...
        try {
          ...
          let recommendPattern =
            /<li class="list-comic" data-key="[^"]*">\s*<a class="ImgA" href="([^"]+)"><mip-img src="([^"]+)"[^>]*alt="([^"]*)"[^>]*><\/mip-img><\/a>\s*<a class="txtA" href="[^"]+">([^<]+)<\/a>/g;
      ...
      try {
        ...
        if (!comicInfo.cover)
          comicInfo.cover = "https://m.ykmh.net/images/default/cover.png";
        if (!comicInfo.tags || !Array.isArray(comicInfo.tags))
          comicInfo.tags = [];
        let updateInfo = "暂无更新";
        try {
          ...
          if (YKMHSource.comic_status && comicInfo.status) {
            ...
            if (!mappedStatus) {
              if (comicInfo.status.includes("连载")) {
                ...
                comicInfo.status.includes("暂停") ||
                comicInfo.status.includes("休刊")
              ) {
                mappedStatus = "paused";
              } else {
    ...
    loadEp: async (comicId, epId) => {
      ...
      if (!chapterUrl.startsWith("http")) {
        chapterUrl =
          "https://m.ykmh.net" +
          (chapterUrl.startsWith("/") ? chapterUrl : "/" + chapterUrl);
          ...
          try {
            ...
            images = imageList.map((img) => {
              if (!img.startsWith("http")) {
                return (
                  "https://m.ykmh.net" + (img.startsWith("/") ? img : "/" + img)
                );
              }
              return img;
            });
