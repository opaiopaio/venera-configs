class MH1234 extends ComicSource {
  ...
  // update url
  url = "https://cdn.jsdelivr.net/gh/opaiopaio/venera-configs@main/mh1234.js";
  ...
  parseComics(html, onePage = false) {
    ...
    return {
      ...
      maxPage: onePage
        ? 1
        : parseInt(doc.querySelector("#total-page").attributes["value"]),
    };
  }
    ...
    load: async (category, params, options, page) => {
      if (params.endsWith(".html")) {
        ...
        const res = await Network.get(
          `${this.baseUrl}/list/?filter=${params}-${options[0]}-${options[1]}-${options[2]}&sort=${options[3]}&page=${page}`,
        );
        console.warn(
          `${this.baseUrl}/list/?filter=${params}-${options[0]}-${options[1]}-${options[2]}&sort=${options[3]}&page=${page}`,
        );
        ...
        return {
          ...
          maxPage: parseInt(
            doc.querySelector("#total-page").attributes["value"],
          ),
        };
      }
    },
  ...
  search = {
    load: async (keyword, options, page) => {
      const res = await Network.get(
        `${this.baseUrl}/search/?keywords=${keyword}&sort=${options[0]}&page=${page}`,
      );
      if (res.status !== 200) {
        throw `Invalid status code: ${res.status}`;
      }
  ...
  comic = {
    loadInfo: async (id) => {
      ...
      const cover = doc.querySelector(".pic").querySelector("img").attributes[
        "src"
      ];
      ...
      const chapters = {};
      const chapterElements = doc
        .querySelector(".chapter-warp")
        ?.querySelectorAll("li");
      if (chapterElements) {
        for (let ch of chapterElements) {
          const id = ch
            .querySelector("a")
            .attributes["href"].replace("/comic/", "")
            .replace(".html", "")
            .split("/")
            .join("_");
    ...
    loadEp: async (comicId, epId) => {
      ...
      const res = await Network.get(
        `${this.baseUrl}/comic/${ids[0]}/${ids[1]}.html`,
      );
      if (res.status !== 200) {
        throw `Invalid status code: ${res.status}`;
      }
